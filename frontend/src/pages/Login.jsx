...
const Login = ({ loginEmail, setLoginEmail, loginPassword, setLoginPassword, loginError, handleLogin }) => {
  return (
      ...
      <div className="w-full max-w-md space-y-6 rounded-2xl border border-gray-200 bg-sportify-white p-8 shadow-md">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input type="email" required value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} className="mt-1 w-full rounded-xl border border-gray-300 bg-sportify-light p-2.5 text-sm outline-none focus:border-sportify-blue" placeholder="admin@sportify.com" />
          </div>
          <div>
          ...
          <button type="submit" className="w-full rounded-xl bg-sportify-blue p-3 text-sm font-bold text-white hover:bg-sportify-deepSea transition-colors shadow-sm">Autenticar Administrador</button>
        </form>
      </div>
    </div>
  );
